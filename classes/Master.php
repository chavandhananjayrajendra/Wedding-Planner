...
Class Master extends DBConnection {
	...
	function save_shop_type(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Service Type already exists.";
		}else{
			if(empty($id)){
			...
			if($save){
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = " New Service Type successfully saved.";
				else
				$resp['msg'] = " Service Type successfully updated.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_shop_type(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Service Type successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function add_to_cart(){
		...
		if($save){
			$resp['status'] = 'success';
			$resp['msg'] = " Service has added to cart.";
		}else{
			$resp['status'] = 'failed';
	...
	function update_cart_qty(){
		...
		if($update_cart){
			$resp['status'] = 'success';
			$resp['msg'] = ' Service Quantity has updated successfully';
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = ' Service Quantity has failed to update';
			$resp['error'] = $this->conn->error;
		}
	...
	function place_order(){
		...
		if(!$has_failed){
			$resp['status'] = 'success';
			$resp['msg'] = " your request for the service has been received successfully.Please wait for some time,we will let you know soon about your request";
			$this->conn->query("DELETE FROM `cart_list` where client_id ='{$this->settings->userdata('id')}'");
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " Service Request Failed";
			$resp['error'] = $this->conn->error;
			if(count($inserted) > 0){
	...
	function cancel_order(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Service has been cancelled successfully.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
